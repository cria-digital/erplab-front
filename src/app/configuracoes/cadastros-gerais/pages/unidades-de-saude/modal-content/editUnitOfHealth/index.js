...
const EditUnityOfHealth = ({ onClose, findData, unit }) => {
  ...
  // Informações básicas
  const [name, setName] = useState(unit?.nomeUnidade)
  const [internalCode, setInternalCode] = useState(unit?.codigoInterno)
  const [cnpj, setCnpj] = useState(unit?.cnpj)
  const [corporateReason, setCorporateReason] = useState(unit?.razaoSocial)
  const [fantasyName, setFantasyName] = useState(unit?.nomeFantasia)
  ...
  const [contacts, setContacts] = useState(unit?.contatosUnidade)
  const [email, setEmail] = useState(unit?.email)
  const mainServicesCodes = [
    {
      id: '4.02',
      label:
        '4.02 - ANÁLISES CLINICAS, PATOLOGIA, ELETRICIDADE MÉDICA, RADIOTERAPIA',
    },
  ]

  const [mainServiceCode, setMainServiceCode] = useState(
    mainServicesCodes.find((i) => i.id === unit?.codigoServicoPrincipal) || '',
  )

  const secondaryServiceCodes = [
    {
      id: '4.03',
      label:
  ...
  const [selectSecondaryServiceCode, setSelectSecondaryServiceCode] = useState(
    secondaryServiceCodes.filter((s) =>
      unit?.codigoServicoSecundario?.includes(s.id),
    ),
  )
  ...
  // Endereço
  const [cep, setCep] = useState(unit?.cep)
  const [street, setStreet] = useState(unit?.rua)
  const [number, setNumber] = useState(unit?.numero)
  const [district, setDistrict] = useState(unit?.bairro)
  const [complement, setComplement] = useState(unit?.complemento)
  const [city, setCity] = useState(unit?.cidade)
  const [state, setState] = useState(unit?.estado)
  ...
  // Responsáveis
  const [responsibleName, setResponsibleName] = useState(unit?.nomeResponsavel)
  ...
  // Impostos
  const [IRRF, setIRRF] = useState(unit?.irrfPercentual)
  const [PIS, setPIS] = useState(unit?.pisPercentual)
  const [COFINS, setCOFINS] = useState(unit?.cofinsPercentual)
  const [CSLL, setCSLL] = useState(unit?.csllPercentual)
  const [ISS, setISS] = useState(unit?.issPercentual)
  const [IBS, setIBS] = useState(unit?.ibsPercentual)
  const [CBS, setCBS] = useState(unit?.cbsPercentual)
  const [retainISS, setRetainISS] = useState(unit?.reterIss)
  const [retainIR, setRetainIR] = useState(unit?.reterIr)
  const [retainPCC, setRetainPCC] = useState(unit?.reterPcc)
  const [retainIBS, setRetainIBS] = useState(unit?.reterIbs)
  const [retainCBS, setRetainCBS] = useState(unit?.reterCbs)
  ...
  const [financial, setFinancial] = useState(() => {
    const mapped = (unit?.dadosBancarios ?? []).map((i) => ({
      banco: i.banco?.nome ?? '',
      codigoBanco: i.banco?.codigo ?? '',
      bancoId: i.bancoId ?? i.banco?.id ?? '',
      agencia: i.agencia ?? '',
